...
#### Negative/positive numeric rendering — registry variants

A single "negative = red" treatment is too blunt across a real fintech app
— a −0.03 and a −792 don't carry the same weight, and a negative value
isn't always a problem (a hedge position, for example, is *expected* to run
negative). The registry should offer named variants, chosen per column by
`renderAs`/`dataType`, not decided ad hoc in each renderer:

| `renderAs` | Behavior | Best for |
|---|---|---|
| `signedNumber` | Plain minus sign, no color judgment | Values where sign carries no business meaning |
| `accountingNumber` | Parentheses for negative `(123.00)`, muted color, no severity | Default numeric formatting — calm baseline |
| `trend` | Icon (↑/↓) + accounting format + severity-driven color | Default for "change" columns (diff %, P&L) |
| `trendMagnitude` | Same as `trend`, but color/weight intensity scales with size | Columns where the *size* of the move matters (VaR diff) |
| `divergingBar` | Small bar growing left (negative) / right (positive) from center | Fast row-by-row scanning across many rows |
| `heatmapCell` | Whole-cell background tint, not just text | High-signal columns only — heavy if repeated |
| `monochromeTrend` | Icon-only direction, no red/green at all | Accessibility fallback / colorblind-safe mode |
| `deltaPill` | Compact rounded badge, e.g. summary cards | Single highlight column, not dense repeated columns |
| `strikethroughReversed` | Muted strikethrough text | Voided/reversed/cancelled entries specifically (not "bad" values) |

**Key rule learned from real dashboards:** reserve strong color/pill
treatment for rows where `severity` is explicitly flagged (a real breach or
outlier) — small or expected negative values should render as plain muted
text (`accountingNumber`), not the same loud color as a genuine problem.
Coloring every negative number identically flattens the grid's visual
hierarchy and makes real outliers harder to spot at a glance.
...
## 9. Mock `gridData` — service-side example

This mirrors a real exposure grid (Level 5 / Region / Desk breakdown with
Delta/Gamma measures), showing how `renderAs`, `dataType`, and `severity`
combine — including the "only flag real outliers" rule from Phase 3.
...
```json
{
  "rows": [
    {
      ...
      "level5": "Equity Cash",
      "region": "AMER",
      "volckerDesk": "Equity Cash NA",
      "level8": "US Cash Trading",
      "delta": -32.69,
      "gamma": 1.03,
      "severity": "danger"
    },
    {
      ...
      "region": "AMER",
      "volckerDesk": "Equity Cash NA",
      "level8": "Central Risk Desk (CRD) NAM",
      "delta": -0.03,
      "gamma": null,
      "severity": "none"
    },
  ...
  "metadata": {
    ...
    "rowIdField": "id",
    "pagination": { "pageSize": 50 },
    "groupBy": ["level5", "region", "volckerDesk", "level8"]
  }
}
```

**Notes on this example:**
- `row-001` has `severity: "danger"` — this is the only row that should
  render with the strong pill/background treatment from the `trend`
  renderer; everything else has `severity: "none"` and renders as plain
  muted `accountingNumber` text, even though `row-002`'s delta is also
  negative.
- `gamma: null` (row-002, row-004) represents "not applicable" — the
  `trend` renderer should render an em-dash or blank, not a false `0.00`.
- `gamma: 0` (row-003) is a real zero value and should render as `0.00`,
  distinct from `null`.
- `groupBy` in metadata reflects the drag-to-group column behavior seen in
  the reference dashboard — this is a Phase 7 (interaction polish) feature,
  config-driven per grid rather than hardcoded.
